;; Use heap insted of vector for multiway-merge-sort
#lang r7rs
...
(define-library (multiway-merge-sort)
  ...
  (import (scheme base)
          ...
          (prefix (a-d heap standard) heap:) ;; ADDED
  ...
  (begin
    ...
    (define rlen 10)
    (define irun (heap:new rlen <)) ;; MODIFIED
      ...
      (let loop
        ...
        (cond ((or (= indx rlen) (not (in:has-more? file)))
              ...
              (else
               ...
               (heap:insert! irun (in:read file)) ;; ADDED
               (loop (+ indx 1))))))
    ...
    (define (write-run! ofcr imax)
      (let loop
        ...
        (ofcr:write! ofcr (heap:delete! irun)) ;; ADDED
        (if (< (+ indx 1) imax)
            (loop (+ indx 1)))))
    ...
    (define (distribute! inpt files <<?)
      ...
      (let loop
        ...
        (let ((nmbr (read-run! inpt)))
          (when (not (= nmbr 0))
            #| (quicksort irun nmbr <<?) |# ;; The heap does te order
